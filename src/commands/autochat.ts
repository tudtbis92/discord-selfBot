	...
	execute: async (agent: BaseAgent, msg: Message, ...args: string[]) => {
		if (!args.length) {
			...
			const embed = new MessageEmbed()
				...
				.addField("Chat cuối", stats?.lastChatTime ? `<t:${Math.floor(stats.lastChatTime / 1000)}:R>` : "Chưa có", true)
				.addField("Chat tiếp theo", stats?.timeUntilNextChat && stats.timeUntilNextChat > 0 ? 
					`${Math.ceil(stats.timeUntilNextChat / 60000)} phút nữa` : "Sẵn sàng", true)
				...
				.addField("Sử dụng", 
					...
					`\`${agent.config.prefix}autochat test\` - Test gửi tin nhắn ngay`, false);
		...
		switch (subCommand) {
			...
			case "enable":
				if (!args[1]) {
					return msg.reply("❌ Vui lòng cung cấp Channel ID!\nSử dụng: `autochat on <channelId>`");
				}
				...
				const channelName = channel.type === "DM" ? "DM Channel" : (channel as any).name || "Unknown";
			...
			case "interval":
				if (!args[1] || isNaN(parseInt(args[1]))) {
					return msg.reply("❌ Vui lòng cung cấp số phút hợp lệ!\nSử dụng: `autochat interval <phút>`");
				}
