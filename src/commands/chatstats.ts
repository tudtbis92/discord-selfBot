...
export const chatstats: Commands = {
	...
	execute: async (agent: BaseAgent, msg: Message, ...args: string[]) => {
		if (!agent.autoChatManager) {
			return msg.reply("❌ Auto Chat Manager chưa được khởi tạo!\nSử dụng `setchat` để thiết lập.");
		}
		...
		if (stats.enabled) {
			embed.addField("🕐 Chat cuối cùng", 
				stats.lastChatTime ? `<t:${Math.floor(stats.lastChatTime / 1000)}:R>` : "Chưa có", true);
			...
			embed.addField("🎯 Đang xử lý mention", stats.isProcessingMention ? "✅ Có" : "❌ Không", true);
		}

		// Thêm thông tin cấu hình
		embed.addField("⚙️ Cấu hình",
			`**Channel ID:** \`${agent.config.autoChatChannelID || "Chưa thiết lập"}\`\n` +
			`**Auto Chat:** ${agent.config.autoChat ? "✅ Bật" : "❌ Tắt"}\n` +
			`**Interval:** ${agent.config.autoChatInterval || 4} phút`, false);

		// Thêm hướng dẫn nhanh
		if (args[0] !== "minimal") {
			embed.addField("🛠️ Lệnh quản lý",
				...
				`\`${agent.config.prefix}chat <tin nhắn>\` - Test chat AI`, false);
		}
