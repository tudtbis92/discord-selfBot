...
const joinvCommand: Commands = {
	name: 'joinv',
	description:
		'Join bot vào kênh voice được chỉ định. Sử dụng: joinv <channelId> hoặc joinv <guildId> <channelId>',
	execute: async (agent, message, ...args) => {
		try {
			...
			if (args.length > 1 && args[1]) {
				...
				if (!voiceChannel) {
					await message.reply(
						`❌ Không tìm thấy kênh voice với ID ${channelId} trong server ${guild.name}!`,
					);
					return;
				}
			} else {
				...
				if (!voiceChannel) {
					...
					logger.info(
						`[JoinV] Không tìm thấy kênh trong cache, đang fetch từ các server...`,
					);
					...
					for (const guild of agent.guilds.cache.values()) {
						try {
							...
							logger.debug(
								`[JoinV] Không thể fetch channels từ server ${guild.name}: ${error}`,
							);
						}
					}
				}
			...
			if (!voiceChannel) {
				await message.reply(
					'❌ Không tìm thấy kênh voice với ID đã cung cấp trong bất kỳ server nào!',
				);
				return;
			}
			...
			await message.reply(
				`✅ Đã join vào kênh voice: **${voiceChannel.name}**${guildId ? ` trong server **${voiceChannel.guild?.name}**` : ''}`,
			);
		} catch (error) {
