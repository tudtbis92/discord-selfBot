		...
		try {
			...
			for (const guild of guildsToCheck) {
				try {
					...
					logger.debug(
						`[ListV] Đã fetch ${guild.channels.cache.size} channels từ ${guild.name}`,
					);
					...
					logger.debug(
						`[ListV] Tìm thấy ${voiceChannels.size} voice channels trong ${guild.name}`,
					);
					...
					logger.error(
						`[ListV] Không thể fetch channels từ server ${guild.name}: ${error}`,
					);
				}
			}
